...
Spec.templates["level"]={
  type: "string",
  html: `<div class="title">${L("LEVEL")}</div>
  <div class="fy_container">
    <div class="fy_box">
      <div class="fy_horizon">
        <span class="fy_textbox" style="position: absolute; left: 0px; right: 0px;">
          <select onchange="Fy.changed()">
            <option value="0">${L("private note, not shown on extranets")}</option>
            <option value="1">${L("private note, shown on extranets")}</option>
            <option value="2">${L("public note")}</option>
          </select>
        </span>
      </div>
    </div>
