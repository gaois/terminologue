//A script to fix banjaxed "CYBERSECURITY" database (zfmjx6yjk)
//Based off of upgrade-2020-01-03.js, A script to migrate all SQLite databases when the database schema has changed.
...
function migrateDBs(dirPath){
  var filenames=fs.readdirSync(dirPath).filter(filename => /zfmjx6yjk\.sqlite$/.test(filename));
